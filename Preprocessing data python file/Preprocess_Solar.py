...
# Configuration for solar power to do preprocess
...
if not os.path.exists(INPUT_FILENAME):
    ...
    # clean enough to load directly
    df = pd.read_csv(INPUT_FILENAME)
    print(f"Successfully loaded {len(df):,} rows.")

    
    # 2. Creating a Single Timestamp Column 
    
    print("\n-Step 2: Combining Year, Month, Day, Hour, Minute into a single timestamp ")
    
    # This one is to create a dictionary of the time columns to pass to pd.to_datetime
    # a robust way to build a timestamp from separate columns.
    time_cols = {
        'year': df['Year'],
        'month': df['Month'],
        'day': df['Day'],
    ...
    # Setting the new timestamp as the index for time series operations
    df.set_index('timestamp', inplace=True)
    
    # to ensure the data is sorted by time, crucial for resampling
    ...
    # Resample the GHI data to hourly. The standard method is to take the average GHI over the hour.
    solar_hourly = solar_df.resample('h').mean()
    # After resampling, there might still be gaps if a whole hour was missing so we can fill these again to ensure a continuous time series.
    ...
    # this one is to rename the column for consistency with our final GP model
    ...
    # to save the final, clean dataframe to a new CSV file.
    solar_hourly.to_csv(OUTPUT_FILENAME, index=False)

    print("\nPreprocessing complete!")
    print(f"Clean solar data saved. Now we are ready to merge this file with our workload data.")
    print("\nFinal Data Head:")
