...
import os
import pickle
#  1: Configuration for a SINGLE Model Run for wind
#pre-configured to run for your Wind data. ---
INPUT_FILE = 'preprocessed_wind_hourly.csv'
TARGET_COLUMN_NAME = 'wind_power_mw' 
...
if not os.path.exists(INPUT_FILE):
    ...
    print("Using a Wind Power Kernel (Daily Cycle + RBF)...")
    kernel = (
    gpflow.kernels.SquaredExponential(variance=0.1, lengthscales=5.0) +
    gpflow.kernels.Periodic(gpflow.kernels.SquaredExponential(), period=24.0)
)
...
# Create the EFFICIENT Sparse GP model
    ...
    # Step 5: Visualize the Final Result (this one is optional)
    ...
    def plot_gp(gp_model, y_mean, y_std, title):
        ...
        plt.ylabel("Value (Original Scale - Wind MW)")
        plt.legend()
        plt.grid(True)
        # plt.show() 
