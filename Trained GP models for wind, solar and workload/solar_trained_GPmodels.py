...
#  Configuration for a SINGLE Model Run (Solar) 

INPUT_FILE = 'preprocessed_solar_hourly.csv'
TARGET_COLUMN_NAME = 'solar_mwh' # This should match the column in your preprocessed solar file
# save model file
...
if not os.path.exists(INPUT_FILE):
   ...
   #single data set loading
    ...
    # Add a small epsilon to std dev to prevent division by zero if all values are the same ( all 0 at night)
...
# Create the EFFICIENT Sparse GP model
    ...
    def plot_gp(gp_model, y_mean, y_std, title):
        ...
        plt.ylabel("Value (Original Scale - GHI)")
        plt.legend()
        plt.grid(True)
        # plt.show() 
